...
try {
    ...
    if (pages.findIndex(component => component === pageName) !== -1) throw new Error('Страница уже существует');
    ...
    fs.writeFile(viewsPathJs, viewsJs, err => {
        ...
        fs.mkdir(folderPath, {}, err => {
            ...
            fs.writeFile(tsPath, eval('`' + componentTemplate + '`'), err => {
                ...
                fs.writeFile(pugPath, `extends ../../app/layout\n\nblock title\n    | ${pageTitle}\n\nblock content\n    main(data-barba="container" data-barba-namespace="${pageName}")`, err => {
                    if (err) throw new Error(err.message);
